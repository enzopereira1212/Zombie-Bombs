...
local database = DataStoreService:GetDataStore("Run")
local playersData = {}
...
local function onRequestPowerUpgrade(player: Player)
	...
	if data.gold < UPGRADE_COST then
		...
		MarketplaceService:PromptProductPurchase(player, 1816835913)
...
local function onEnemyDefeated(playerId: number)
	...
	if leaderstats then
		...
		score.Gold += GOLD_EARNED_ON_ENEMY_DEFEAT
	end
...
MarketplaceService.ProcessReceipt = function(receiptInfo)
	...
	if receiptInfo.ProductId == 1816835913 then
